...
const LAYER_ICONS: Record<string, React.ReactNode> = {
  ...
  'grid-3x3': <Grid3x3 className="w-4 h-4" />,
...
interface StateCardsProps {
  stateName: string;
  stateSlug: string;  // Used for layer ID prefix (e.g., 'india', 'karnataka')
  service: {
    getLayers: () => Promise<StateResponse<StateLayer[]>>;
    getLayerData: (layerId: string) => Promise<StateResponse<GeoJSONFeatureCollection>>;
  };
  attribution?: string;
}

export const StateCards = ({ stateName, stateSlug, service, attribution }: StateCardsProps) => {
  ...
  useEffect(() => {
    const fetchLayers = async () => {
      try {
        const response = await service.getLayers();
        ...
        setError(`Failed to connect to ${stateName} data service`);
      } finally {
  ...
  const handleLoadLayer = async (layer: StateLayer) => {
    setLayerStates(prev => ({
      ...prev,
  ...
  const handleUnloadLayer = (layer: StateLayer) => {
    window.dispatchEvent(new CustomEvent('geo:remove-layer', {
      detail: { id: `${stateSlug}-${layer.id}` }
    }));
